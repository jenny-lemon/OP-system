"""
opapp.py  ──  營運報表控制台 v6
設計方向：清新米白底、高對比深色文字、柔和藍綠強調色
新功能：
  - 排程主控表直接 inline 編輯排程時間（每行有 ✏️ 編輯按鈕）
  - 任務今日執行狀態（根據 last_run 日期自動判斷，顯示「今日成功」）
  - 每個任務可啟用 / 停用（checkbox toggle）
  - Log 監控支援關鍵字搜尋與高亮
  - 手動執行可輸入額外臨時參數
  - 批次控制列加「全部每日」快捷鈕
"""
...
DEFAULT_CONFIG = {
    "daily": [
        {"id":"d1","label":"排班統計表",  "script":"schedule_report.py",    "args":[],"schedule":"01:10","all_regions":True, "enabled":True},
        {"id":"d2","label":"專員班表",    "script":"staff_schedule.py",     "args":[],"schedule":"01:20","all_regions":True, "enabled":True},
        {"id":"d3","label":"專員個資",    "script":"staff_info.py",         "args":[],"schedule":"01:30","all_regions":True, "enabled":True},
        {"id":"d4","label":"當月次月訂單","script":"orders_report.py",      "args":[],"schedule":"01:40","all_regions":True, "enabled":True},
        {"id":"d5","label":"業績報表",    "script":"performance_report.py", "args":[],"schedule":"08:00","all_regions":True, "enabled":True},
    ],
    "monthly": [
        {"id":"m1","label":"上半月訂單", "script":"half_month_orders.py",        "args":["1"],"schedule":"月中15日","all_regions":True, "enabled":True},
        {"id":"m2","label":"下半月訂單", "script":"half_month_orders.py",        "args":["2"],"schedule":"月底28日","all_regions":True, "enabled":True},
        {"id":"m3","label":"已退款",     "script":"refund_report.py",            "args":[],   "schedule":"月底28日","all_regions":True, "enabled":True},
        {"id":"m4","label":"預收",       "script":"prepaid_report.py",           "args":[],   "schedule":"月初01日","all_regions":True, "enabled":True},
        {"id":"m5","label":"儲值金結算", "script":"stored_value_settlement.py",  "args":[],   "schedule":"月初01日","all_regions":True, "enabled":True},
        {"id":"m6","label":"儲值金預收", "script":"stored_value_prepaid.py",     "args":[],   "schedule":"月初01日","all_regions":True, "enabled":True},
    ],
    "log_files": {},
}
...
def record_run(key, ok, stdout, stderr):
    log = load_runlog()
    log[key] = {"last_run": now_tw().strftime("%Y-%m-%d %H:%M:%S"), "ok": ok,
                "stdout": (stdout or "")[-1500:], "stderr": (stderr or "")[-1500:]}
    save_runlog(log)
...
def run_script(script, args=None, region=None):
    ...
    try:
        r = subprocess.run(cmd, capture_output=True, text=True, timeout=300, cwd=str(BASE_DIR), env=env)
        return {"ok": r.returncode==0, "cmd":" ".join(cmd), "stdout": r.stdout, "stderr": r.stderr}
...
def scan_output():
    ...
    for p in sorted(OUTPUT_DIR.rglob("*")):
        if p.is_file():
            ...
            files.append({"name":p.name,"path":p,
                "folder": str(p.parent.relative_to(OUTPUT_DIR)) if p.parent!=OUTPUT_DIR else "根目錄",
                "size_kb":round(s.st_size/1024,1),"mtime":mdt,
                "mtime_str":mdt.strftime("%Y-%m-%d %H:%M"),"today":mdt.date()==today})
...
def calc_next_run(s: str) -> str:
    ...
    try:
        ...
        import re
        mt = re.search(r"(\d{1,2})日", s)
        day = int(mt.group(1)) if mt else {"月初":1,"月中":15,"月底":28}.get(s[:2])
        if day:
            c = now.replace(day=day,hour=1,minute=0,second=0,microsecond=0)
            if c<=now:
...
chip = (f'<span class="rgn-chip">📍 {rgn}</span>'
...
def region_selector(allow_all=False):
    if not ACCOUNTS:
        ...
        return None
    opts = ["（不指定地區）"]
    if allow_all: opts.append(REGION_ALL)
    opts += REGIONS
    ...
    with ci:
        if sel not in ("（不指定地區）",REGION_ALL):
            if sel in ACCOUNTS: st.info(f"✅ **{sel}** 帳號：`{ACCOUNTS[sel].get('email','—')}`")
            else: st.warning(f"⚠️ 找不到「{sel}」帳密")
        elif sel==REGION_ALL: st.info(f"🌐 將依序執行：{', '.join(REGIONS)}")
        else: st.caption("未指定地區時腳本不會收到帳密環境變數")
...
def show_result(label, pairs):
    for rn, res in pairs:
        pre = f"**{rn}** · " if rn!="—" else ""
        if res["ok"]: st.success(f"✅ {pre}{label} 完成")
        else: st.error(f"❌ {pre}{label} 失敗")
        with st.expander(f"{'✅' if res['ok'] else '❌'} {pre}輸出詳情", expanded=not res["ok"]):
            st.caption(f"`{res.get('cmd','')}`")
            if res.get("stdout"): st.text_area("stdout", res["stdout"], height=170, key=f"so{label}{rn}{id(res)}")
            if res.get("stderr"): st.text_area("stderr", res["stderr"], height=130, key=f"se{label}{rn}{id(res)}")
...
if page == "排程主控表":
    ...
    def render_section(grp_key, title, sec):
        ...
        for idx,job in enumerate(jobs):
            ...
            if   ok is True and today_ran: st_b = badge("green","今日成功")
            elif ok is True:               st_b = badge("teal","✓ 成功")
            elif ok is False and today_ran:st_b = badge("red","今日失敗")
            elif ok is False:              st_b = badge("red","✗ 失敗")
            else:                          st_b = badge("gray","待執行")
            ...
            with row[0]:
                ...
                if chk and enabled: checked.append(job)
            ...
            with row[3]:
                if st.session_state.get(edit_key,False):
                    nv = st.text_input("", value=job.get("schedule",""), key=f"sv_{job['id']}",
                                       placeholder="HH:MM 或 月初01日", label_visibility="collapsed")
    ...
    with cc1:
        if st.button("🔄 重新整理", use_container_width=True): st.rerun()
    with cc2:
        if st.button("▶ 執行勾選", use_container_width=True, type="primary"):
            if not all_checked: st.warning("請先勾選至少一個任務")
            else:
    ...
    # Failures
    failures = [(j, runlog.get(rkey(j["id"],region)) or runlog.get(j["id"]))
                for j in all_jobs
    ...
    if failures:
        ...
        for job,entry in failures:
            with st.expander(f"❌ {job['label']}  ·  {entry.get('last_run','—')}"):
                if entry.get("stderr"): st.code(entry["stderr"], language="bash")
                if entry.get("stdout"): st.code(entry["stdout"])
        st.markdown("</div>", unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════
    ...
    def render_manual(jobs, title):
        ...
        for job in jobs:
            ...
            with c2:
                ...
                nr = calc_next_run(sv)
                st.caption(f"排程：{sv}")
                if nr!="—": st.caption(f"↻ {nr}")
            with c3:
                ea = st.text_input("額外參數（選填）","",key=f"ea_{job['id']}",
                                   placeholder="如 202504 或留空", label_visibility="visible")
            with c4:
    ...
    def batch(jobs,title):
        for job in [j for j in jobs if j.get("enabled",True)]:
            ...
            show_result(job["label"], pairs)
    with bc1:
        if st.button("▶ 全部每日", use_container_width=True): batch(cfg["daily"],"每日")
    with bc2:
        if st.button("▶ 全部月報", use_container_width=True): batch(cfg["monthly"],"月報")
    with bc3:
        if st.button("▶ 全部任務", use_container_width=True): batch(cfg["daily"]+cfg["monthly"],"全部")
    st.markdown("</div>", unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════
    ...
    with c1: sel_log = st.selectbox("選擇 Log 檔", list(builtin_logs.keys()), key="log_sel")
    with c2: n_lines = st.selectbox("行數", [50,100,200,500], index=1, key="log_n")
    with c3: kw = st.text_input("🔍 關鍵字篩選 / 高亮", placeholder="輸入後 Enter…", key="log_kw")
    with c4:
        st.markdown("<br>", unsafe_allow_html=True)
    ...
    if sel_log.startswith("📋"):
        ...
        if lpath.exists():
            try:
                ...
                rows = [{"任務 key":k,"最後執行":v.get("last_run","—"),
                ...
                with st.expander("完整 JSON"): st.json(data)
            except Exception as e:
                st.error(f"無法解析：{e}")
        else:
            st.info("尚無執行紀錄")
    ...
    with st.form("add_log"):
        l1,l2 = st.columns(2)
        with l1: alias = st.text_input("別名", placeholder="例：cron.log")
        with l2: fpath = st.text_input("完整路徑", placeholder="/path/to/file.log")
        if st.form_submit_button("➕ 加入"):
            if alias and fpath:
                cfg["log_files"][alias]=fpath; save_config(cfg); st.success(f"已加入：{alias}"); st.rerun()
            else: st.error("必填")
        ...
        for nm in list(extra.keys()):
            ...
            with r2:
                if st.button("移除", key=f"rm_{nm}"):
                    del cfg["log_files"][nm]; save_config(cfg); st.rerun()
    st.markdown("</div>", unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════
    ...
    if today_files:
        ...
        st.dataframe(pd.DataFrame([{"檔名":f["name"],"資料夾":f["folder"],"時間":f["mtime_str"],"大小 KB":f["size_kb"]} for f in today_files[:30]]),
    ...
    with cf: sf = st.selectbox("資料夾", ["（全部）"]+folders, key="of")
    with ck: kw2 = st.text_input("🔍 搜尋檔名", placeholder="關鍵字…", key="ok")
    with cr:
        st.markdown("<br>", unsafe_allow_html=True)
        if st.button("🔄 掃描", use_container_width=True): st.rerun()

    filtered = [f for f in files if (sf=="（全部）" or f["folder"]==sf) and (not kw2 or kw2.lower() in f["name"].lower())]
    if not filtered:
        st.info("沒有符合條件的檔案")
    else:
        st.dataframe(pd.DataFrame([{"檔名":f["name"],"資料夾":f["folder"],"修改時間":f["mtime_str"],"大小 KB":f["size_kb"],"今日":"✅" if f["today"] else "—"} for f in filtered]),
                     use_container_width=True, hide_index=True)
    st.markdown("</div>", unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════
        ...
        with tab:
            ...
            for idx,job in enumerate(jobs):
                ...
                with st.expander(f"{icon} {job['label']}  ·  {job.get('schedule','—')}  ·  {en_lbl}"):
                    ...
                    with c2:
                        ...
                        nt = st.text_input("排程時間（HH:MM 或 月初NN日）", job.get("schedule",""), key=f"sch_{job['id']}")
                    c3,c4 = st.columns(2)
                    with c3: nm = st.checkbox("需跑全部地區", value=job.get("all_regions",False), key=f"mul_{job['id']}")
                    with c4: ne = st.checkbox("啟用此任務",   value=job.get("enabled",True),      key=f"en2_{job['id']}")
                    nr = calc_next_run(nt)
                    if nr!="—": st.markdown(f'<div class="next-run">⏭️ 下次執行預估：<strong>{nr}</strong></div>', unsafe_allow_html=True)
                    a1,a2,a3 = st.columns(3)
                    with a1:
                        if st.button("💾 儲存", key=f"sv_{job['id']}"):
                            cfg[grp_key][idx].update({"label":nl,"script":ns,"args":na.split() if na.strip() else [],
                                                       "schedule":nt,"all_regions":nm,"enabled":ne})
                            save_config(cfg); st.success("✅ 已儲存"); st.rerun()
                    with a2:
                        if st.button("▶ 測試", key=f"ts_{job['id']}"):
                            with st.spinner("…"): pairs=do_run_job(job,st.session_state.region)
                            show_result(job["label"],pairs)
                    with a3:
                        if st.button("🗑️ 刪除", key=f"dl_{job['id']}"):
                            cfg[grp_key].pop(idx); save_config(cfg); st.warning(f"已刪除 {job['label']}"); st.rerun()
            st.markdown("</div>", unsafe_allow_html=True)
            ...
            with st.form(f"add_{grp_key}"):
                ...
                with c1:
                    ...
                    fs = st.text_input("腳本檔名", placeholder="例：新報表.py")
                with c2:
                    fa = st.text_input("固定參數",  placeholder="留空或 1")
                    ft = st.text_input("排程時間",  placeholder="08:00 或 月初01日")
                fc1,fc2 = st.columns(2)
                with fc1: fm = st.checkbox("需跑全部地區")
                with fc2: fen = st.checkbox("啟用", value=True)
                if st.form_submit_button("➕ 新增"):
                    if not fl or not fs: st.error("名稱與腳本必填")
                    else:
                        cfg[grp_key].append({"id":new_id(grp_key[0]),"label":fl,"script":fs,
                                              "args":fa.split() if fa.strip() else [],
                                              "schedule":ft or "—","all_regions":fm,"enabled":fen})
                        save_config(cfg); st.success(f"✅ 已新增：{fl}"); st.rerun()
            st.markdown("</div>", unsafe_allow_html=True)
        ...
        if sel and sel!="（請選擇）":
            ...
            with e1:
                if st.button("💾 寫入儲存", type="primary", use_container_width=True):
                    sp.write_text(edited, encoding="utf-8"); st.success(f"✅ 已寫入 {sel}")
            with e2:
                if st.button("🔄 重新讀取", use_container_width=True): st.rerun()
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    with tab_r:
        ...
        if st.checkbox("✅ 確認重置"):
            if st.button("🔄 執行重置"):
                save_config(DEFAULT_CONFIG); st.warning("已重置，請重新整理"); st.rerun()
        st.markdown("</div>", unsafe_allow_html=True)


# ── Footer ────────────────────────────────────────
st.markdown("<br>", unsafe_allow_html=True)
st.caption(f"營運報表控制台 v6  ·  {now_tw().strftime('%Y-%m-%d %H:%M:%S')} 台北  ·  可用地區：{', '.join(REGIONS) if REGIONS else '尚未設定'}")

...
def render_order_date_report_tab():
    ...
    st.caption("預設起迄日皆為當日，依訂購日期查詢，結果依地區統計，待付款、已付款分成兩張表；每張底下再依服務日期拆出固定的月份欄位（本月＋4個月），金額都是後端財務彙總表直接算好的含稅總金額（已扣車馬費），跟地區/加總欄位同一套邏輯、口徑一致。儲值金（跟「目前總表」用同一套判斷邏輯，非用儲值金付款的清潔訂單）獨立成每張表各自的「儲值金待付款/儲值金已付款」欄位，不拆月份；家電（冷氣＋洗衣機）、水洗、收納同樣各自獨立成「家電待付款/家電已付款」「水洗待付款/水洗已付款」「收納待付款/收納已付款」欄位，不拆月份、也不再算進一般待付款/已付款。")

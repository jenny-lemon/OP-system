...
'''
if old not in s:
    raise SystemExit('weekend surcharge target not found')
...
if count < 1:
    raise SystemExit('order-date parse_html call not found')
block = block.replace(old_call, 'parse_html(response.text, payment_status=status)')
